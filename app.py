...
def select_and_balance_match():
    ...
    def best_split(four, strict_partner=True):
        """在指定的4人中，找出「未曾同隊」優先、其次「戰力差最小」的分隊方式。
        strict_partner=True 時，過濾掉同隊級數差超過 MAX_PARTNER_LEVEL_DIFF 的組合。"""
        best = None
    ...
    valid_matches = []       # 同隊差 ≤ 3 且跨隊差 ≤ 3
    relaxed_matches = []     # 同隊差 ≤ 3 但跨隊差 > 3
    fallback_matches = []    # 完全放寬（同隊差也可能 > 3）

    for four in itertools.combinations(sorted_players, 4):
        # 嚴格模式：同隊級數差 ≤ MAX_PARTNER_LEVEL_DIFF
            ...
            with st.form("edit_player_form"):
                ...
                with save_col2:
                    delete_btn = st.form_submit_button(
                        "🗑️ 刪除球員", use_container_width=True
                    )
                ...
                if update_btn:
                    if not new_name:
                        ...
                        and new_name in st.session_state.players
                    ):
                        ...
                        for c_id, match in st.session_state.courts.items():
                            if match:
                                if selected_player in match["team1"]:
                                    ...
                                    match["sum1"] = sum(
                                        st.session_state.players[p]["level"]
                        ...
                        for key, count in st.session_state.partner_history.items():
                            if selected_player in key:
                                key = frozenset(
                                    new_name if p == selected_player else p
                                    for p in key
                                )
